...
if (isset($_POST['form_action']) && $_POST['form_action'] == 'create_msa') {
    ...
    $events = isset($_POST['technicalEvents']) ? implode(", ", $_POST['technicalEvents']) : 'No events selected';
    $teamMembers = isset($_POST['teamMembers']) && !empty($_POST['teamMembers']) ? htmlspecialchars($_POST['teamMembers']) : 'N/A';

    try {
       $query = "INSERT INTO technical_data (name, branch, year, admissionId, phone, email, events, team_members) VALUES (:name, :branch, :year, :admissionId, :phone, :email, :events, :team_members)";
